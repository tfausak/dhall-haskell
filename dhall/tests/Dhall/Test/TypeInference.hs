...
successTest prefix = do
    ...
    Test.Util.testCase prefix skip $ do
        ...
        tryResolvedExpr <- Exception.try (Test.Util.load (Core.denote actualExpr))
        ...
        -- Note that we cannot access the expected type by importing it since
        -- might not be well-typed.
        expectedTypeCode <- Text.IO.readFile (Text.unpack prefix <> "B.dhall")
